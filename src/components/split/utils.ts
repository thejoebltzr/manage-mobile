import {SplitFactory as SplitSdk} from '@splitsoftware/splitio-react-native';
...
export interface IFactoryWithClients extends SplitIO.ISDK {
  ...
  config: SplitIO.IReactNativeSettings;
}

// exported for testing purposes
export const __factories: Map<
  SplitIO.IReactNativeSettings,
  IFactoryWithClients
> = new Map();
...
// idempotent operation
export function getSplitSharedClient(
  ...
  const client = factory.client(key) as IClientWithContext;
...
export function destroySplitFactory(
  factory: IFactoryWithClients,
): Promise<void[]> {
  // call destroy of shared clients and main one
  const destroyPromises = [];
  factory.sharedClientInstances.forEach(client =>
...
function validateSplit(
  ...
  if (maybeSplit === undefined) {
    console.log(
      `[ERROR] you passed a null or undefined ${item}, ${item} must be a non-empty string.`,
    );
  } else if (!isString(maybeSplit)) {
    console.log(
      `[ERROR] you passed an invalid ${item}, ${item} must be a non-empty string.`,
    );
  } else {
    if (TRIMMABLE_SPACES_REGEX.test(maybeSplit)) {
      console.log(
        `[WARN] ${item} "${maybeSplit}" has extra whitespace, trimming.`,
      );
    ...
    if ((maybeSplit as string).length > 0) {
      ...
      console.log(
        `[ERROR] you passed an empty ${item}, ${item} must be a non-empty string.`,
      );
    }
  }
